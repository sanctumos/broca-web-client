...
- Direct route handlers (lines 66, 68, 99, 105, 111, 117, 123, 128, 138)
- Error handling paths (lines 152, 185-186, 191, 244, 259, 304, 347, 352, 357, 384-385, 407-408, 413, 426-427, 432, 454-455, 465, 487-492, 552-553)
"""
...
class TestAPIDirectRoutesComprehensive:
    """Test all direct route handlers for complete coverage"""
    ...
    def test_handle_clear_data_direct_success(self, app):
        """Test direct clear_data route - covers line 128"""
        with app.test_client() as client:
    ...
    def test_handle_cleanup_logs_direct_success(self, app):
        """Test direct cleanup_logs route - covers line 138"""
        with app.test_client() as client:
            with patch('app.api.routes.handle_cleanup_logs') as mock_handle:
                mock_handle.return_value = {'success': True, 'message': 'Success'}
...
class TestAPIErrorPathsComprehensive:
    ...
    def test_handle_messages_rate_limit_exceeded(self, app):
        """Test messages rate limiting - covers line 185-186"""
        with app.test_client() as client:
    ...
    def test_handle_inbox_method_not_allowed(self, app):
        """Test inbox method validation - covers line 352"""
        with app.test_client() as client:
    ...
    def test_handle_outbox_method_not_allowed(self, app):
        """Test outbox method validation - covers line 384-385"""
        with app.test_client() as client:
    ...
    def test_handle_outbox_auth_failure(self, app):
        """Test outbox authentication failure - covers line 407-408"""
        with app.test_client() as client:
            with patch('app.api.routes.require_auth_internal') as mock_auth:
                mock_auth.return_value = {'success': False, 'error': 'Auth failed'}
    ...
    def test_handle_outbox_missing_fields(self, app):
        """Test outbox missing fields validation - covers line 426-427"""
        with app.test_client() as client:
            response = client.post('/api/v1/?action=outbox',
                                json={'session_id': ''},
                                headers={'Authorization': 'Bearer test_key'})
    ...
    def test_handle_outbox_database_exception(self, app):
        """Test outbox database exception handling - covers line 454-455"""
        with app.test_client() as client:
            with patch('app.api.routes.get_db') as mock_get_db:
                mock_db = MagicMock()
    ...
    def test_handle_sessions_method_not_allowed(self, app):
        """Test sessions method validation - covers line 487-492"""
        with app.test_client() as client:
    ...
    def test_handle_config_method_not_allowed(self, app):
        """Test config method validation - covers line 552-553"""
        with app.test_client() as client:
...
class TestAPIEdgeCasesComprehensive:
