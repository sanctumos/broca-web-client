"""
Unit tests for widget routes to achieve 100% coverage
"""
...
class TestWidgetRoutes:
    ...
    def test_widget_init_success(self):
    ...
    def test_widget_init_missing_api_key(self):
    ...
    def test_widget_init_default_values(self):
        """Test widget init route with default values only"""
        with patch('app.widget.routes.request') as mock_request, \
             patch('app.widget.routes.jsonify') as mock_jsonify, \
             patch('app.widget.routes.datetime') as mock_datetime:
            
            # Mock request arguments - only API key provided
            mock_request.args.get.side_effect = lambda key, default=None: {
                'apiKey': 'test-api-key'
            }.get(key, default)
            ...
            assert call_args['data']['config']['position'] == 'bottom-right'
            assert call_args['data']['config']['theme'] == 'light'
            assert call_args['data']['config']['title'] == 'Chat with us'
            assert call_args['data']['config']['primaryColor'] == '#007bff'
            assert call_args['data']['config']['language'] == 'en'
            assert call_args['data']['config']['autoOpen'] is False
            assert call_args['data']['config']['notifications'] is True
            assert call_args['data']['config']['sound'] is True
    ...
    def test_widget_config(self):
        ...
        with patch('app.widget.routes.jsonify') as mock_jsonify, \
            ...
            assert 'bottom-right' in call_args['data']['positions']
            assert 'light' in call_args['data']['themes']
            assert 'en' in call_args['data']['languages']
            assert call_args['data']['defaults']['position'] == 'bottom-right'
    ...
    def test_widget_init_boolean_conversion(self):
        """Test boolean conversion in widget init"""
        with patch('app.widget.routes.request') as mock_request, \
             patch('app.widget.routes.jsonify') as mock_jsonify, \
             patch('app.widget.routes.datetime') as mock_datetime:
            
            # Mock request arguments with various boolean strings
            mock_request.args.get.side_effect = lambda key, default=None: {
                'apiKey': 'test-api-key',
                'autoOpen': 'TRUE',
                'notifications': 'FALSE',
